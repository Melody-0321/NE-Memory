...
- 核心模块风险分级：
  ...
  - 🟡 中风险：state-pipeline, stm-pipeline, ltm-pipeline, consolidate, embedding, template-defs, adaptive-context（自适应上下文裁剪，影响注入策略）, state-toggle（State 抽取总开关/会话级开关，门控 state 抽取与注入）
