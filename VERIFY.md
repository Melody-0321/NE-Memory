...
| S3 | 预设字段定义完整 | `ALL_PREDEFINED_FIELDS` 16 字段含 5 个 category（identity/psychology/social/combat/items/legacy），每字段含 `type/layer/category/description` | ✅ 已通过 |
...
#### Phase 5-6：UI 层（方案编辑器 + 模板管理 — 未实施）

以下来自设计文档 [open-character-schema-ui-plan.md](file:///d:/SillyTavern/xm/ne-memory/.trae/documents/open-character-schema-ui-plan.md)，当前代码中**未实施**或仅有骨架桩：
...
| SU8 | 版本历史查看与回退 | 模板详情页中版本时间线 → 回退到任意历史版本 | ⚠️ UI 存在，无实际交互 |
...
| Phase 6 | 用户编辑器 UI + 引导 UI（方案编辑器、模板库 slide-in、tooltip、版本历史） | ⚠️ 骨架桩（panel-templates.js 存在但内容不完整） |
| Phase 7 | 测试 + 兼容迁移 | ✅ 已实施 |
...
| # | 问题 | 根因 | 影响 | 状态 |
|---|------|------|------|:--:|
| B1 | 模板库面板为空（全局库 + 角色卡库均无默认模板） | `panel-templates.js` 的 `renderTemplatesIntoSlide()` 调用 `loadTemplateLibrary()`（读 localStorage `ne_template_library`），但默认模板 `_default_pc` / `_default_npc` 仅在 `schema.js` 代码常量中，从未写入 localStorage。管线侧 `resolveFieldDef()` 有两层回退，但 UI 侧没有。 | 用户打开模板库时看到空状态"还没有模板"，无法选择/管理默认模板。管线运行时不受影响（静默自启走代码常量）。 | 🔴 待修复 |
| B2 | SU8 模板版本历史有 UI 但无交互 | `panel-templates.js` 中版本时间线渲染了但绑定事件未实现或后端函数未就绪 | 用户看到版本记录但无法点击回退 | 🟡 待修复（低优先级） |
...
| # | 项目 | 方法 |
|---|------|------|
| P2-1 | `rollbackState` 可用 | 需手动触发回退并验证 State 回到旧值 | ⏳ 待验证 |
| P2-2 | `rollbackMemory` 可用 | 同上 | ⏳ 待验证 |
...
| U3 | compact 压缩 | 累计 100+ delta 后自动 fold | ⏳ 待验证 |

### Split Vault — 拆分存储

| # | 项目 | 方法 |
|---|------|------|
| S1 | `state_vaults` 有数据 | F12 → Application → 点开看 key = chat_id | ✅ 已通过 |
| S2 | `memory_vaults` 有数据 | 同上 | ✅ 已通过 |
| S3 | 旧 `vaults` 数据迁移成功 | State 面板角色状态有内容；记忆面板有旧条目 | ✅ 已通过 |
...
| R3 | 模板 LLM | Function Calling 可用，新 NPC 出现时在管线中触发 `get_character_scheme` tool | ⏳ 待验证（需新 NPC 进入场景） | |
...
```
Schema-S1: Function Calling ✅ 已通过 — 副 API 正常工作

P1-2+P1-3: ✅ 已验证 — 管线运行时 recordStateDelta + recordMemoryVersion 正常

P2-1~P2-3: 回滚功能？
  ├─ 是 → 继续
  └─ 否 → 需排查 rollbackState/rollbackMemory/restoreBranch
```
...
```
1. ~~F12 → Application → IndexedDB → 确认 P1-1 + Split-S1/S2~~ ✅
2. ~~Verify data constants + architecture~~ ✅ Phase 1-4 + S1-S4 ✅
3. ~~State 面板 → 角色状态有数据？记忆面板 → 有条目？~~ ✅
4. ~~发一轮对话 → 控制台检查 P1-2/P1-3 日志~~ ✅
5. ~~版本历史面板 → 确认 U1~~ ✅
6. ~~继续 3-5 轮 → 记忆面板确认有新条目~~ ✅
```
