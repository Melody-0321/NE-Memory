...
// 构成：21 锚 = 12 状态锚（7 来自 t0-state-arcs + 5 新构造演化链）+ 9 叙述锚（qa13-18 原批 + qa19-21 扩容批），共 76 facts（state 36 + narrative 40）。
...
//
// 【扩容批次 2026-08-19】（narrative-anchor-expansion-plan.md §2.1/§2.2）：
//   narrative tier 判定单元 n=18 → 40：新增 qa19/qa20/qa21（来自未锚定 dev narrative 查询 q1/q2/q15）
//   + qa13/qa14/qa15 各扩 2 facts。选点规则（outcome-blind）：queries.js 预注册 groundTruth
//   按 weight 降序、事件 id 升序、跳过锚内既有 facts 已覆盖的事件。
//   事后重叠登记：构造时臂结果已公开（canonical §8 的 6/46 缺失）。6 个缺失事件
//   {stm_51, stm_24, stm_44, stm_36, stm_37, stm_40} 中落入本批新 facts 承载事件的为
//   stm_24、stm_44（qa20，q2 与 q12 GT 结构性重叠所致），共 2/22 < 3/22 降档阈值，措辞不降档。
//   方向提醒：该重叠使覆盖缺口点估计存在上偏风险（已知缺失事件更易产出 coverfix−baseline 正差），decision.md 已如实标注。
...
export var qaAnchors = [
  ...
  { // q3 林晚人物 —— 601/楼下∈stm_08 ✓ 酱油∈stm_08 ✓ 通过∈stm_48 ✓ 毕业论文/瓶颈∈stm_18 ✓ 陈教授/严格∈stm_19 ✓（扩容批次）
    id: 'qa13', type: 'narrative', source: 'q3',
    query: '林晚是一个什么样的角色？',
    gtEventIds: ['stm_08', 'stm_18', 'stm_19', 'stm_48'],
    facts: [
      { q: '林晚住在哪？', expect: ['601', '楼下'] },
      { q: '林晚和江岚是怎么认识的？', expect: ['酱油'] },
      { q: '林晚的毕业论文最后结果如何？', expect: ['通过'] },
      { q: '林晚来还酱油的时候顺便吐槽了什么？', expect: ['毕业论文', '瓶颈'] },
      { q: '林晚的论文导师是谁？对她怎么样？', expect: ['陈教授', '严格'] },
    ],
  },
  { // q18 苏茉作用 —— 行政∈stm_58 ✓ 合作∈stm_13 ✓ 王姐∈stm_99 ✓ 复杂/欣赏/不想输∈stm_12 ✓ 在乎/认个错∈stm_116 ✓（扩容批次）
    id: 'qa14', type: 'narrative', source: 'q18',
    query: '苏茉在整个故事中起了什么作用？',
    gtEventIds: ['stm_12', 'stm_13', 'stm_58', 'stm_99', 'stm_116'],
    facts: [
      { q: '苏茉辞职前是做什么工作的？', expect: ['行政'] },
      { q: '苏茉建议安然对江岚采取什么态度？', expect: ['合作'] },
      { q: '苏茉的读书社区项目是和谁一起合作的？', expect: ['王姐'] },
      { q: '安然曾向苏茉倾诉过怎样的感情？', expect: ['复杂', '欣赏', '不想输'] },
      { q: '苏茉在电话里是怎么点拨江岚的？', expect: ['在乎', '认个错', '认错'] },
    ],
  },
  { // q12 感情转折 —— 可乐∈stm_24 ✓ 安然∈stm_44 ✓ 夕阳∈stm_46 ✓ 披/毯子∈stm_10 ✓ 并肩/谁都没说话/站了很久∈stm_25 ✓（扩容批次）
    id: 'qa15', type: 'narrative', source: 'q12',
    query: '两个人感情中的关键转折在哪里？',
    gtEventIds: ['stm_10', 'stm_24', 'stm_25', 'stm_44', 'stm_46'],
    facts: [
      { q: '安然情绪低落时江岚给她递了什么？', expect: ['可乐'] },
      { q: '是谁先说出"我在乎的是你"的？', expect: ['安然'] },
      { q: '两人第一次牵手是在什么场景下？', expect: ['夕阳'] },
      { q: '江岚趴在书桌上睡着后，安然悄悄做了什么？', expect: ['披', '毯子'] },
      { q: '安然接过可乐后，两人在阳台上是什么状态？', expect: ['并肩', '谁都没说话', '没说话', '站了很久'] },
    ],
  },
  ...
  // ═══════════ 叙述锚扩容批（2026-08-19，narrative-anchor-expansion-plan.md §2.2）═══════════

  { // q1 月票榜起因过程 —— 质疑/运气∈stm_03 ✓ 借口/别找∈stm_05 ✓ 三百∈stm_15 ✓ 反超/爆更/五百∈stm_21 ✓ 众筹∈stm_22 ✓
    id: 'qa19', type: 'narrative', source: 'q1',
    query: '月票榜事件的起因和过程？',
    gtEventIds: ['stm_03', 'stm_05', 'stm_15', 'stm_21', 'stm_22'],
    facts: [
      { q: '月票榜大战最初是因为什么挑起来的？', expect: ['质疑', '运气'] },
      { q: '江岚应战时反过来挑衅了安然什么？', expect: ['借口', '别找'] },
      { q: '月票榜大战中期谁一度领先？领先多少票？', expect: ['三百', '300'] },
      { q: '月票榜的战况后来是怎么被扭转的？', expect: ['反超', '爆更', '五百'] },
      { q: '江岚的读者们做了什么帮她冲榜？', expect: ['众筹'] },
    ],
  },
  { // q2 关系发展 —— 眼眶/微红∈stm_24 ✓ 没有道谢/谁都没说话∈stm_25 ✓ 安然主动∈stm_28 ✓ 不在乎∈stm_44 ✓ 我也是/不一样/跟别人∈stm_45 ✓ 两个主角/结局∈stm_46 ✓
    id: 'qa20', type: 'narrative', source: 'q2',
    query: '两人之间的关系是如何发展的？',
    gtEventIds: ['stm_24', 'stm_25', 'stm_28', 'stm_44', 'stm_45', 'stm_46'],
    facts: [
      { q: '江岚递可乐的时候安然是什么状态？', expect: ['眼眶', '微红'] },
      { q: '安然对江岚递来的可乐是什么反应？', expect: ['没有道谢', '没道谢', '没有拒绝', '没拒绝', '没说话'] },
      { q: '是谁先提出别比了、合作写一本书的？', expect: ['安然主动', '主动推开', '主动提出', '安然'] },
      { q: '安然在阳台上向江岚坦白了什么？', expect: ['不在乎', '在乎的是你'] },
      { q: '听到坦白后江岚是怎么回应的？', expect: ['我也是', '不一样', '跟别人'] },
      { q: '安然说合作小说的结局应该怎么安排？', expect: ['两个主角', '结局'] },
    ],
  },
  { // q15 争吵与和解 —— 新大纲/茶几∈stm_122 ✓ 铅笔/写给/合作比竞争∈stm_123 ✓ 温度∈stm_124 ✓ 默契/配合∈stm_125 ✓ 创作方向/科幻/都市情感∈stm_113 ✓
    id: 'qa21', type: 'narrative', source: 'q15',
    query: '安然出来后带来了什么？他们怎么解决的？',
    gtEventIds: ['stm_122', 'stm_123', 'stm_124', 'stm_125', 'stm_113'],
    facts: [
      { q: '安然从卧室出来后拿出了什么？', expect: ['新大纲', '茶几'] },
      { q: '大纲最后一页的空白处写了什么？', expect: ['铅笔', '写给', '合作比竞争'] },
      { q: '江岚为什么说这是她读过的最好的大纲？', expect: ['温度'] },
      { q: '两人重新画大纲时配合得怎么样？', expect: ['默契', '配合'] },
      { q: '两人第一次真正因为什么吵的架？', expect: ['创作方向', '科幻', '都市情感'] },
    ],
  },
];
