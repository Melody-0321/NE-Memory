...
(function() {
    ...
    for (var i = 1; i <= 95; i++) {
        ...
        seed[d] = { tok_stm: 10, tok_consolidate: 0, tok_state: 0, tok_tool: 0, tok_chat: 0 };
    }
...
(function() {
    ...
    recordDailyToken('tok_consolidate', 200);
    ...
    eq(overview.breakdown.consolidate, 200, '场景2: breakdown.consolidate 即时可见');
    eq(overview.breakdown.chat, 300, '场景2: breakdown.chat 即时可见');
    eq(overview.todayNE, 301, '场景2: todayNE = stm+consolidate');
...
(function() {
    ...
    eq(persisted[TODAY].tok_consolidate, 200, '场景3: flush 后 tok_consolidate 落盘');
