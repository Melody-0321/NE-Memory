...
- **版本历史主动重抽（State / Memory）**：版本历史时间线上，State 的 `ai_update` 与 Memory 的 `stm_batch` 条目新增「🔄 重抽」按钮 —— 撤销该次 AI 抽取并重新提取，全程不改动对话内容。底层用 append-only 补偿版本（`state_reroll` / `stm_reroll`），不截断版本链，因此重抽本身也能在版本历史里回退，连续重抽天然可重入且不累积漂移。冲突保护：角色卡正则写入（`ne_char_update`）的字段自动排除在回溯范围外；`manual_edit` 冲突时弹窗逐类告知（精确到字段/条目，旧记录作粗粒度提示），由用户选择覆盖或取消，取消则零写入
- **State / Memory 页新增快捷重抽入口**：无需翻版本历史，两个标签页的版本控制条上直接点「🔄 重抽」即可重抽**最近一次 AI 抽取**（等价于时间线上最新那条 AI 版本的重抽）。版本游标不在链头（刚点过「回退」、vault 处于折叠态）时按钮置灰并提示先点「前进」回到最新，避免在错位基准上重抽
- **State 抽取开关（总开关 + 会话级）**：State 管线原先每轮必跑且无法关闭，是唯一「每轮必花」的固定 LLM 调用。新增设置面板总开关与 State 标签页的会话级开关（跟随全局 / 本会话开 / 本会话关，会话级优先，默认跟随全局）；关闭后同时停止状态抽取与注入并清除残留注入，STM/LTM 与记忆账本不受影响；重新打开后从下一轮起立即恢复更新，重开首轮不注入（避免喂入关闭前的旧状态）

## 性能优化

- **优化：处理历史批跑提速与流式产出**：每个抽取 chunk 完成即写入 vault 并刷新「待补摘要楼层」列表（原来攒到末尾一次性落盘）；批量回填跳过反悔消解调用，LLM 调用量约减半；进度改为 chunk 级实时刷新（原来只显示 0 和完成），按钮运行中带呼吸态、默认批大小 20 → 8（vNext-59）
- **优化：通知弹窗使用 ST 原生 toastr**：toast 提示优先走 SillyTavern 全局 toastr（与 ST 生态样式一致），检测不到时降级回内置样式（vNext-59）

## Bug 修复

- **修复：副 API 连不上不开 CORS 的端点（如基元律动）**：副 API 原为浏览器直连目标端点，依赖对方开启跨域；基元律动等未开 CORS 即连不上、模型列表也拉不到 → 改为走 SillyTavern 服务端后端路由转发（`/api/backends/chat-completions/generate` / `status`），由 ST 服务端发起请求，无 CORS 限制、无需任何额外配置，模型列表同步修复（vNext-61）
- **修复：次级 API 连接失败时无法看到服务端错误详情**：非 2xx（如 403）响应此前只显示 `API error: 403`，服务端 body（错误码 + traceId）被丢弃，无法判断是 key 无效 / 余额不足 / 风控 / 模型无权限 → 现在把网关错误码与 message 带进报错信息，连接测试与调用日志均可直接看到（vNext-60）
- **修复：处理历史按钮净化后仍未生效（无空缺也触发 / 卡顿依旧）**：去重 id 裸下标与 vault 全格式 id 不匹配，导致全部消息被当未处理 → 统一改用 `buildMsgId`，并新增「待补摘要楼层」折叠区直观展示未处理楼层；无空缺时预扫描短路不再弹确认框（vNext-58）
- **修复：处理历史/整合按钮卡顿**：处理历史串行全量补摘阻塞 UI 且不可取消 → 改后台批量补摘（进度 + 取消 + 断点续跑 + 批间让出管线队列，批大小改按消息数可调）；"整合"按钮与自动整合行为重复，冗余移除（vNext-57）

---

# NE-Memory v8.0.0 更新日志

## 架构升级

- **存储权威化**：vault 持久化从 IndexedDB 迁移到 ST 聊天文件（聊天文件恒胜），跨浏览器/跨设备共享记忆；记忆加载走权威读路径，本地副本不再权威
- **密钥安全**：secret 密钥/凭证类字段（ne_*_api）不再写入服务器 extensionSettings，保持 localStorage 私有，杜绝密钥泄露（历史版本曾把 API key 一并镜像进服务端）

## 升级注意事项

- **数据自动迁移，无需手动备份**：首次升级后触发记忆读写时，若聊天文件还没有记忆数据，NE 会自动从 IndexedDB 回填，此后记忆随聊天文件跨浏览器/跨设备共享
- **记忆不再存于浏览器本地**：v8.0 起聊天文件是记忆的唯一权威，本地 IndexedDB 仅作读写穿透缓存、不参与权威裁决；此前的版本/哈希/LWW 双存储裁决已移除
- **多端差异**：同时用多个浏览器/设备时，以「最后一次写入聊天文件」的端为准；新端首次打开以本地缓存回填，可能与其它端存在差异
