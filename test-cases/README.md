...
> v2 架构：三层判决分割（L1 硬性规则 + L2 词汇连贯性 + L3 LLM 边裁）→ 已切区间 → Phase 2 批量摘要 LLM。
> `status` 统一为 `"closed"`，不再有 `"partial"` 概念。每个测试用例都应验证三层覆盖分布。

| # | 用例 | 状态 | 描述 |
|---|------|------|------|
| 10 | **基础提取（续连续场景）** | 🟡 待测 | 同一场景中连续 7 轮对话，检查：L1/L2/L3 边界裁决分布（预期 L1+L2 >70%），Phase 2 摘要的 event 质量、turns 范围、实体标注 |
| 11 | **场景切换提取** | ⬜ 未定义 | 对话中自然切换场景（地点/时间跳转），L1 是否捕获场景切换信号（`sameChar`），边界不被漏切 |
| 12 | **多角色引入提取** | ⬜ 未定义 | 3+ 角色参与对话，L1 `sameChar` 是否正确检测角色变化并切出新事件；entity 标注覆盖所有参与角色 |
| 13 | **长时间线 batch 提取** | ⬜ 未定义 | 长对话（15-20 轮）触发的 batch pipeline，确保 L3 边裁调用不超时、Phase 2 token 不溢出、分批正确 |
| 14 | **三层判决分级分布** | ⬜ 未定义 | 从 trace 统计 L1/L2/L3 的边界裁决占比：L1（硬性规则）预期 >30%，L1+L2（代码总计）预期 >70%，L3（LLM 边裁）预期 <30% |
| 15 | **L3 边裁准确度** | ⬜ 未定义 | 对 L3 模糊区间（`overlay` 0.1-0.5），检查 LLM yes/no 回答的准确率。构造已知边界的对话对，逐对验证 |
| 16 | **Phase 2 批量摘要质量** | ⬜ 未定义 | 已切好的区间传入 Phase 2 后，检查：每个区间产生恰好 1 条 STM，event 文本聚焦该区间内容，不跨区间引用 |
| 17 | **pipeline 失败后自动恢复** | ⬜ 未定义 | Phase 2 摘要 LLM 调用失败时（API 错误/超时），无 STM 产生（安全失败）；下轮 pipeline 是否正常重试 |
| 18 | **text-utils 分词一致性** | ⬜ 未定义 | `text-utils.js` 的 `tokenize` 输出与旧 `retrieval-filter.js` 内联版本完全一致（中文 2-gram + 英文小写分词）；`vocabularyOverlap` 在完全相同文本上返回 ≥0.95 |
