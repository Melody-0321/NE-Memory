...
semantic:
  - "注入内容是否以 HTML 结构化格式组织（h2 关键记忆 → h3 实体名 (N events) → 条目列表）？"
  - "注入中每条实体链标题行是否带命中/总数计数（N events in chain, M hits）？"
  - "注入是否包含 h2 关键记忆 段，汇总当前轮最相关的记忆条目？"
  - "注入内容是否避免了乱码、截断或重复碎片？"
minRounds: 4
maxRounds: 10
expectedRounds: "5-7"
...
1. **实体链分块** — `h2 关键记忆` 下按实体分 `h3 实体名 (N events in chain, M hits)` 块
2. **命中计数** — 每个实体链标题行带 `N events in chain, M hits` 统计
3. **关键记忆** — 开头 `h2 关键记忆` 汇总最相关的 top-5 记忆
4. **未标注条目** — 无实体归属的条目独立展示为 `h3 未标注条目`
...
### 语义性断言
1. 注入内容是否以 HTML 结构化格式组织（h2 关键记忆 → h3 实体名 → 条目列表）？
2. 注入中每条实体链是否带命中/总数计数在链标题行？
3. 注入是否包含 h2 关键记忆 段？
