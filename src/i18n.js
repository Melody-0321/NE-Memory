...
export const NARRATIVE_I18N = {
    'en': {
        ...
        'State extraction failed': 'State extraction failed', 'All messages have already been processed.': 'All messages have already been processed.',
        ...
        'Memory extraction uses LLM to detect natural scene boundaries, not fixed message counts. This is only a hard cap — unprocessed messages beyond this force extraction. A low value makes it behave like a fixed threshold.': 'Memory extraction uses LLM to detect natural scene boundaries, not fixed message counts. This is only a hard cap —  unprocessed messages beyond this force extraction. A low value makes it behave like a fixed threshold.',
    ...
    'zh-cn': {
        ...
        'State extraction failed': '状态提取失败', 'All messages have already been processed.': '所有消息均已被处理。', 'Consolidation failed': '整合失败',
        ...
        'Memory extraction uses LLM to detect natural scene boundaries, not fixed message counts. This is only a hard cap — unprocessed messages beyond this force extraction. A low value makes it behave like a fixed threshold.': '记忆提取由 LLM 自动判断场景边界，并非按固定消息数切分。此处仅为硬上限——未处理消息超出此数时强制提取。设得较低时近似固定阈值触发。',
        'Connect': '连接', 'Test Message': '测试消息',
        'Not connected': '未连接', 'Connected': '已连接', 'Connecting...': '连接中...',
    ...
    'zh-tw': {
        ...
        'State extraction failed': '狀態提取失敗', 'All messages have already been processed.': '所有消息均已被處理。', 'Consolidation failed': '整合失敗',
        ...
        'Memory extraction uses LLM to detect natural scene boundaries, not fixed message counts. This is only a hard cap — unprocessed messages beyond this force extraction. A low value makes it behave like a fixed threshold.': '記憶提取由 LLM 自動判斷場景邊界，並非按固定訊息數切分。此處僅為硬上限——未處理訊息超出此數時強制提取。設得較低時近似固定閾值觸發。',
        'Connect': '連接', 'Test Message': '測試訊息',
        'Not connected': '未連接', 'Connected': '已連接', 'Connecting...': '連接中...',
        'API connection successful!': 'API 連接成功！',
        'API connection failed. Check browser console (F12) for details.': 'API 連接失敗。請查看瀏覽器主控台（F12）以獲取詳情。',
