...
export async function renderUsageTab() {
    ...
    html += '<div class="ne-usage-section">' +
        '<div class="ne-usage-section-title">\u{1F4CA} ' + t('Current Chat') + ' / ' + t('Today') + ' / ' + t('This Month') + '</div>' +
        '<div class="ne-usage-cards">' +
        '<div class="ne-usage-card"><div class="ne-usage-card-value">\u21BB ' + fmt(overview.sessionTotal) + '</div><div class="ne-usage-card-sub">\u2699 ' + t('NE Pipeline') + ': ' + fmt(overview.sessionNE) + ' | \u{1F464} ' + t('User Chat') + ': ' + fmt(overview.sessionChat) + '</div><div class="ne-usage-card-sub">' + (overview.sessionTurns || 0) + ' ' + t('Turns') + ' | ' + t('Avg / Turn') + ': ' + fmt(overview.sessionAvgPerTurn) + '</div></div>' +
        '<div class="ne-usage-card"><div class="ne-usage-card-value">\u2600 ' + fmt(overview.todayTotal) + '</div><div class="ne-usage-card-sub">\u2699 ' + t('NE Pipeline') + ': ' + fmt(overview.todayNE) + ' | \u{1F464} ' + t('User Chat') + ': ' + fmt(overview.todayChat) + '</div><div class="ne-usage-card-sub">&nbsp;</div></div>' +
        '<div class="ne-usage-card"><div class="ne-usage-card-value">\u{1F4C5} ' + fmt(overview.monthTotal) + '</div><div class="ne-usage-card-sub">\u2699 ' + t('NE Pipeline') + ': ' + fmt(overview.monthNE) + ' | \u{1F464} ' + t('User Chat') + ': ' + fmt(overview.monthChat) + '</div><div class="ne-usage-card-sub">' + (overview.monthDays || 0) + ' ' + t('Days') + ' | ' + t('Avg / Day') + ': ' + fmt(overview.monthAvgPerDay) + '</div></div>' +
        '</div></div>';

    /* Section B: Pipeline breakdown — pie chart with scope dropdown */
    html += '<div class="ne-usage-section">' +
        '<div class="ne-usage-section-title">\u{1F4CA} ' + t('Pipeline Breakdown') + '</div>' +
